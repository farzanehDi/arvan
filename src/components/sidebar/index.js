    ...
    return (
        <>
            <div className="relative min-h-screen md:flex" data-dev-hint="container">
                ...
                <aside id="sidebar"
                    className="z-10 pt-20 bg-blue text-white md:w-64 w-2/4 absolute inset-y-0 left-0 transform md:relative md:translate-x-0 transition duration-200 ease-in-out flex flex-col overflow-y-auto"
                    data-dev-hint="sidebar; px-0 for frameless; px-2 for visually inset the navigation">
                    <p className={'ml-5 mb-4 text-xl'}>Post</p>
                    <NavLink end to={"/articles"} className={({isActive}) => `py-2.5 pl-8 ${isActive ? " bg-white/[.2]":""}`}>All Articles</NavLink>
                    <NavLink to={"/articles/create"} className={({isActive}) => `py-2.5 pl-8 ${isActive ? " bg-white/[.2]":""}`}>New Article</NavLink>
                </aside>
